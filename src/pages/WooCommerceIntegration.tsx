...
const getWooCommerceIntegrationDefaults = (lang: 'tr' | 'en') => (
    lang === 'tr'
        ? {
            ...
            ctaTitle: 'WooCommerce Sitenizi Hemen Bağlayın',
            ctaDescription: 'Ücretsiz üye olun ve WooCommerce entegrasyonunu dakikalar içinde aktif edin.',
            ctaButtonText: 'Ücretsiz Üye Ol',
            seoTitle: 'WooCommerce Kargo Entegrasyonu | AdorelGo',
            seoDescription: 'WordPress WooCommerce sitenizi AdorelGo ile entegre edin. Otomatik sipariş ve kargo yönetimi.',
            sections: [
                {
                    type: 'card-grid',
                    cards: [
                        { icon: 'fa-sync-alt', title: 'Otomatik Sipariş Senkronizasyonu', description: 'WooCommerce mağazanızdaki siparişler otomatik olarak AdorelGo paneline aktarılır.' },
                        { icon: 'fa-mouse-pointer', title: 'Tek Tıkla Gönderi Oluşturma', description: 'Siparişlerinizi tek bir tıklamayla kargoya verin.' },
                        { icon: 'fa-truck', title: 'Takip Numarası Otomatik Güncelleme', description: 'Kargo takip numaraları otomatik olarak WooCommerce sitenize iletilir.' },
                    ]
                },
            ],
        }
        : {
            ...
            ctaTitle: 'Connect Your WooCommerce Site Today',
            ctaDescription: 'Sign up for free and activate your WooCommerce integration in minutes.',
            ctaButtonText: 'Sign Up Free',
            seoTitle: 'WooCommerce Shipping Integration | AdorelGo',
            seoDescription: 'Integrate your WordPress WooCommerce site with AdorelGo for automatic order and shipping management.',
            sections: [
                {
                    type: 'card-grid',
                    cards: [
...
const WooCommerceIntegration: React.FC = () => {
    ...
    useEffect(() => {
        axios.get(`${API_BASE_URL}/content/page/woocommerce-entegrasyonu?lang=${currentLang}`)
            .then(r => { if (r.data && Object.keys(r.data).length > 0) setCms(r.data); })
            .catch(() => {})
            .finally(() => setLoading(false));
    }, [currentLang]);

    const defaults = getWooCommerceIntegrationDefaults(currentLang);
    ...
    return (
        <div className="flex flex-col min-h-screen overflow-x-hidden">
            <SEO page="woocommerce-entegrasyonu" customTitle={defaults.seoTitle} customDescription={defaults.seoDescription} />
            <Navbar />
            <main className="flex-grow pt-20">
                ...
                <section className="py-6 bg-gradient-to-r from-[#102477] to-[#1a3a9e] text-white">
                    <div className="max-w-5xl mx-auto px-6 lg:px-8 text-center">
                        <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center mx-auto mb-8"><i className="fab fa-wordpress text-[#4DB848] text-2xl"></i></div>
