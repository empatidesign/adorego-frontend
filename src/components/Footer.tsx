...
const Footer: React.FC = () => {
  ...
  const rawLogoUrl = (bottomSection?.logoUrl || siteSettings?.footerLogo || navbarFooterLogo || '').trim();
  const footerLogoUrl = rawLogoUrl ? (rawLogoUrl.startsWith('http') ? rawLogoUrl : `${API_BASE_URL}${rawLogoUrl}`) : '';
      ...
      <footer className="bg-white text-[#102477] pt-12 pb-12 border-t border-slate-100" role="contentinfo">
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            <p className="text-slate-300 text-[10px] font-bold tracking-widest">{bottomSection.copyrightText || '© 2024 AdorelGo. Site kargo firması vitrini değil, teknoloji lojistik platformudur.'}</p>
            {Array.isArray(bottomSection?.paymentMethods) && bottomSection.paymentMethods.length > 0 ? (
              <div className="flex items-center gap-6 opacity-20">
                {bottomSection.paymentMethods.map((pm: any, idx: number) => (
                  <img key={idx} src={pm.logoUrl?.startsWith('http') ? pm.logoUrl : `${API_BASE_URL}${pm.logoUrl}`} className="h-4 object-contain" alt={pm.name} />
                ))}
              </div>
            ) : (
              <div className="flex items-center gap-6 opacity-20">
